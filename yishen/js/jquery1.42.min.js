...
$(document).ready(function(){
	...
	$container = $('#navbar-search-result');
...
function search(val){
	if(globalData){
		...
		for(var key in globalData){
			if(globalData.hasOwnProperty(key) && val){
				var index = globalData[key].indexOf(val);
				if(~index){
					result.push({
						key: key,
						index: index
...
function searchResult(result){
	...
	if(result.length){
		...
		for(var i = 0;i < result.length;i++){
			var column = result[i].key,
				index  = result[i].index;

			var data = globalData[column].substring(index , index+50);

			if(~data.indexOf('|')){
				data = data.substring(0 ,data.indexOf('|'));
			}
			html.push('<li><a href="./'+column+'.html">');
			html.push('<span class="capitalize">'+(column == 'index' ? 'home' : column)+'</span>');
			html.push('<span>'+data+'</span>');
			html.push('</a></li>');

		}
	}else{
		html.push('not to find');
	}
	html.push('</ul>')
